...
function ueditor_SyntaxHighlighter_print() {
	...
	echo '$(function(){var compatibility={as3:"actionscript","c#":"csharp",delphi:"pascal",html:"markup",xml:"markup",vb:"basic",js:"javascript",plain:"markdown",pl:"perl",ps:"powershell"};if(document.getElementsByClassName){var doms=document.getElementsByClassName("prism-highlight","pre");for(var i=0;i<doms.length;i++){var preDom=doms.item(i);var codeDom=document.createElement("code");codeDom.innerHTML=preDom.innerHTML;console.log(preDom.className);codeDom.className="language-"+function(classObject){if(classObject===null)return"markdown";var className=classObject[0].split("language-")[1];console.log(className);console.log(compatibility[className])?compatibility[className]:className;return compatibility[className]?compatibility[className]:className}(preDom.className.match(/prism-language-([0-9a-zA-Z]+)/gi))+" prism-line-numbers";preDom.innerHTML="";preDom.appendChild(codeDom)}}});';
	echo "\r\n";
