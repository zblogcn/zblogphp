...
CodeMirror.defineMode("vbscript", function() {
  var regexVBScriptKeyword = /^(?:Application|Call|Case|CDate|CreateObject|Clear|CInt|CLng|CDbl|Const|CStr|Description|Dim|Do|Each|Else|ElseIf|((?!Response\.)End)|Err|Error|Explicit|Exit|False|For|Function|If|InStr|IsEmpty|IsNull|InStrRev|Left|LCase|Len|Loop|LTrim|Mid|New|Next|Not|Nothing|Now|Number|On|Option|Private|Public|Preserve|Quit|ReDim|Resume|Right|RTrim|Replace|Response|Request|Select|Server|Set|Sub|Then|To|Trim|True|UBound|UCase|Until|VbCr|VbCrLf|VbLf|VbTab)$/im;
  var regexVBScriptMethod= /^(?:Buffer|Charset|Write|BinaryWrite|Form|QueryString)$/im;
  ...
  return {
    token: function(stream) {
	  ...
	  if(ch == "<"){
		stream.eatWhile(/<script language=["']javascript["'] runat=["']server["']/i);
		stream.eatWhile(/<\/script>/i);
	  }

      if (/\w/.test(ch)) {
        stream.eatWhile(/\w/);
        if (regexVBScriptKeyword.test(stream.current())){return "keyword";}
		else if (regexVBScriptMethod.test(stream.current())){return "attribute";}
		else if (ch == "0" && stream.eat(/x/i)) {
		  stream.eatWhile(/[\da-f]/i);
		  return "number";
		}      
...
CodeMirror.defineMIME("application/x-asp", "vbscript");//{ name: "htmlembedded", scriptingModeSpec:"javascript"});
