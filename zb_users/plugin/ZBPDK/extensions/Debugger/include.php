...
function output_content_debugger($is_admin)
{
    ...
    foreach (get_included_files() as $key => $value) {
        $include_count = $key + 1;
        $include_files .= '<tr><td class="center">' . $include_count . '</td><td>' . $value . '</td></tr>';
    }
    ...
    foreach ($GLOBALS['_query_logs'] as $key => $value) {
        $sql_query .= '<tr><td class="center">' . ($key + 1) . '</td><td>' . htmlentities($value) . '</td></tr>';
    }
...
<div id="debug-container" class="debug-container">
    ...
    <div class="debug-body">
        <ul id="debug-content">
            <li>
                <p><strong>页面执行时间</strong>：<span class="debug-data" style="background:#1e90ff">' . $runtime . ' ms</span></p>
                <p><strong>当前内存占用</strong>：<span class="debug-data" style="background:#32cd32">' . $memory_usage . '</span></p>
                <p><strong>引用的文件数</strong>：<span class="debug-data" style="background:#ff8c00">' . $include_count . '</span></p>
                <p><strong>数据库查询数</strong>：<span class="debug-data" style="background:#ff2b30">' . $sql_count . '</span></p>
            </li>
            <li>
            ...
            <li>
                ...
                <table class="debug-table">
                    ...
                    <tbody>' . get_actived_plugins_debugger($is_admin) . '</tbody>
                </table>
            </li>
        </ul>
    </div>
...
<script src="' . $zbp->host . 'zb_users/plugin/ZBPDK/extensions/Debugger/script.js" type="text/javascript"></script>';
    ...
    unset($memory_usage, $include_files, $include_count, $runtime, $sql_query, $sql_count, $type, $GLOBALS['_query_logs']);
}
                ...
                if ($key == $val) {
                    foreach ($GLOBALS[$key] as $k => $v) {
                        if (function_exists($k)) {
                            $str .= '<tr><td>' . $key . '</td><td>' . $k . '</td><td class="center">' . get_plugin_name_debugger($k) . '</td><td class="center"><span class="debug-plg-detail" interface="' . $key . '" func="' . $k . '" title="查看/隐藏详情">ⅰ</span></td></tr><tr style="display:none"><td colspan="4"></td></tr>';
                        }
                    }
                }
            }
