...
if (strtoupper($_SERVER['REQUEST_METHOD'])=='GET' && isset($_GET['action'])) {
    if ($_GET['action'] == 'detail') {
        ...
        if (strpos($res, 'Exception') !== FALSE &&
            strpos($res, 'Exception') == 0) {
            $res = '<p>' . $res . '</p><p>获取接口详情失败! </p>
                <p><strong>interface</strong>: ' . $_GET['interface'] . '</p>
                <p><strong>func</strong>:' . $_GET['func'] . '</p>';
        }
...
function get_interface_detail($interface_name = '', $func_name = '') {
    ...
    $str .= '<pre class="debug-pre">' .
        htmlspecialchars(
            implode(
                '',
                array_slice(file($filename), $start, $end - $start + 1)
            )
        ) . '</pre>';
