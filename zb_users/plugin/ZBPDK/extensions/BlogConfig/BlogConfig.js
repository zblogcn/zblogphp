...
function run2(e, d, h)
{
	...
	switch (e) {
		case "new":
			...
			f = f + 1;
			if (n == true) {
				if ($("#configt tr.configt-msg").length == 0) {
					$("#configt").append('<tr class="configt-msg"><td></td><td>请保存后再新建</td><td></td></tr>');
				}
				return false
			}
			...
			n = true;
			$("#configt tr").last()[0].scrollIntoView({behavior: "smooth", block: "nearest"});
			break;
		case "edit":
		case "del":
			g.post = $("#ta" + d).val();
