...
if (!$zbp->CheckPlugin('LinkCentre')) {
    $zbp->ShowError(48);
    die();
}
InstallPlugin_LinkCentre();
$act = GetVars('act', 'GET');
$suc = GetVars('suc', 'GET');
...
if ($edit = GetVars('edit', 'GET')) {
    ...
    $delbtn = $mod->Source === 'plugin_LinkCentre' ? '&nbsp;<a title="删除当前模块"
...
<div id="divMain">
  ...
  <div id="divMain2">
    ...
    ?>
      ...
      <p>·默认模板路径：<?php echo LinkCentre_Path("u-temp"); ?></p>
      <!--<p>(暂未实现)自定义模板路径：<?php echo LinkCentre_Path("usr/{$mod->FileName}.li"); ?></p>-->
