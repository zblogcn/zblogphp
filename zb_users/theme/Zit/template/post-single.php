...
<div id="topic">
  <nav id="path" class="kico-guide kico-gap"><a href="{$host}">{$msg.home}</a> | <a href="{$article.Category.Url}">{$article.Category.Name}</a> | <a href="{$article->TimeUrl}">{$article.Time('Y-m-d')}</a></nav>
  <h4>
...
<main id="main">
  ...
  <div id="rel">
    <h3 class="zit kico-sub">{$cfg->RelatedTitle}</h3>
    <ul>
    {if $article.Prev}
    <li class="log">
      <a href="{$article.Prev.Url}" title="{$msg.view}《{$article.Prev.Title}》{$msg.details}">
        <img src="{if $article.Prev.Cover}{$article.Prev.Cover}{else}{$host}zb_users/theme/Zit/style/bg.jpg{/if}" alt="{$article.Prev.Title}" class="cover{if !$article.Prev.Cover} hue{/if}">
        <span class="pane">
    ...
    <li class="log">
      <a href="{$article.Next.Url}" title="{$msg.view}《{$article.Next.Title}》{$msg.details}">
        <img src="{if $article.Next.Cover}{$article.Next.Cover}{else}{$host}zb_users/theme/Zit/style/bg.jpg{/if}" alt="{$article.Next.Title}" class="cover{if !$article.Next.Cover} hue{/if}">
        <span class="pane">
    ...
    {foreach $article.RelatedList as $related}
      <li class="log">
        <a href="{$related.Url}" title="{$msg.view}《{$related.Title}》{$msg.details}">
          <img src="{if $related.Cover}{$related.Cover}{else}{$host}zb_users/theme/Zit/style/bg.jpg{/if}" alt="{$related.Title}" class="cover{if !$related.Cover} hue{/if}">
          <span class="pane">
            ...
            {foreach $related.Tags as $tag}
              ...
              <small class="tag kico-hash">{$tag.Name}</small>
            {/foreach}
            {$tagi=null}
            </span>
          </span>
