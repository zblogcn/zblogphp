...
function ActivePlugin_tpure(){
    ...
    Add_Filter_Plugin('Filter_Plugin_ViewList_Core','tpure_Exclude_Category'); //首页过滤指定分类文章，@zxasd不让加
...
function tpure_GetPagabarAlias($key){
    global $zbp;
    switch ($key) {
        case '‹‹':
            return array($key,'首页','第一页');
        break;
        case '‹':
            return array($key,'上一页','上一页');
        break;
        case '›':
            return array($key,'下一页','下一页');
        break;
        case '››':
            return array($key,'尾页','最后一页');
        break;
        default:
            return array($key,$key,'第'.$key.'页');
    }
}
