...
// $zbp->Config('tpure')->PostList = $al;
// $zbp->Config('tpure')->PostArticle = $aa;
// $zbp->Config('tpure')->PostPage = $ap;
		...
		//--------------------------------------------------------------------------------------
		if($zbp->Config('tpure')->HasKey('PostLISTINFO')){
			$listinfo = json_decode($zbp->Config('tpure')->PostLISTINFO,true);
		}
		$listinfo[] = $_POST;
		...
		$zbp->Config('tpure')->PostLISTINFO = json_encode($listinfo);		//列表辅助信息
		//--------------------------------------------------------------------------------------
		if($zbp->Config('tpure')->HasKey('PostARTICLEINFO')){
			$articleinfo = json_decode($zbp->Config('tpure')->PostARTICLEINFO,true);
		}
		$articleinfo[] = $_POST;
		...
		$zbp->Config('tpure')->PostARTICLEINFO = json_encode($articleinfo);	//文章辅助信息
		//--------------------------------------------------------------------------------------
		if($zbp->Config('tpure')->HasKey('PostPAGEINFO')){
			$pageinfo = json_decode($zbp->Config('tpure')->PostPAGEINFO,true);
		}
		$pageinfo[] = $_POST;
		...
		$zbp->Config('tpure')->PostPAGEINFO = json_encode($pageinfo);		//页面辅助信息
		//--------------------------------------------------------------------------------------
		...
		$zbp->Config('tpure')->PostSHARE = $_POST['PostSHARE'];					//文章第三方分享按钮
...
/*
l_user"]=> string(1) "0" ["l_date"]=> string(1) "0" ["l_cate"]=> string(1) "1" ["l_view"]=> string(1) "1" ["l_cmt"]=> string(1) "1" ["a_user"]=> string(1) "1" ["a_date"]=> string(1) "1" ["a_cate"]=> string(1) "1" ["a_view"]=> string(1) "1" ["a_cmt"]=> string(1) "1" ["p_user"]=> string(1) "1" ["p_date"]=> string(1) "1" ["p_view"]=> string(1) "1" ["p_cmt"]=> string(1) "1"
*/


/*

$zbp->Config('tpure')->PostLIST = array('l_user' => (bool)$_POST['l_user'],
										'l_date' => (bool)$_POST['l_date'],
										'l_view' => (bool)$_POST['l_view'],
										'l_cmt' => (bool)$_POST['l_cmt']
									);

$zbp->Config('tpure')->PostArticle = array('a_user' => (bool)$_POST['a_user'],
										'a_cate' => (bool)$_POST['l_cate'],
										'a_view' => (bool)$_POST['a_view'],
										'a_cmt' => (bool)$_POST['a_cmt']
									);

$zbp->Config('tpure')->PostPage = array('p_user' => (bool)$_POST['p_user'],
										'p_date' => (bool)$_POST['p_date'],
										'p_view' => (bool)$_POST['p_view'],
										'p_cmt' => (bool)$_POST['p_cmt']
									);
...
<dl>
	<form method="post" class="setting">
		...
		<dd>
			...
			<table>
				<tbody>
					...
					<tr>
						...
						<td><input type="text" id="PostLOGO" name="PostLOGO" value="<?php if($zbp->Config('tpure')->PostLOGO){echo $zbp->Config('tpure')->PostLOGO;}else{echo $zbp->host.'zb_users/theme/tpure/style/images/logo.png';} ?>" class="urltext thumbsrc"></td>
			...
			<i class="help"></i><span class="helpcon">上传图片LOGO，关闭图片LOGO则使用文字LOGO(调用网站名称)。<br>图片LOGO支持鼠标滑过显示高光特效，支持开启与关闭。</span>
		</dd>
		<dd>
			...
			<table>
				<tbody>
					...
					<tr>
						...
						<td><input type="text" id="PostFAVICON" name="PostFAVICON" value="<?php if($zbp->Config('tpure')->PostFAVICON){echo $zbp->Config('tpure')->PostFAVICON;}else{echo $zbp->host.'zb_users/theme/tpure/style/images/favicon.ico';} ?>" class="urltext thumbsrc"></td>
						<td><input type="button" class="uploadimg" value="上传"></td>
			...
			<table>
				<tbody>
					...
					<tr>
						...
						<td><input type="text" id="PostTHUMB" name="PostTHUMB" value="<?php if($zbp->Config('tpure')->PostTHUMB){echo $zbp->Config('tpure')->PostTHUMB;}else{echo $zbp->host.'zb_users/theme/tpure/style/images/thumb.png';} ?>" class="urltext thumbsrc"></td>
						<td><input type="button" class="uploadimg" value="上传"></td>
			...
			<table>
				<tbody>
					...
					<tr>
						...
						<td><input type="text" id="PostBANNER" name="PostBANNER" value="<?php if($zbp->Config('tpure')->PostBANNER){echo $zbp->Config('tpure')->PostBANNER;}else{echo $zbp->host.'zb_users/theme/tpure/style/images/banner.jpg';} ?>" class="urltext thumbsrc"></td>
						<td><input type="button" class="uploadimg" value="上传"></td>
...
if(isset($listinfo)){
	...
	foreach($listinfo as $key){
		$str .= '<div class="checkui">用户名<input name="l_user" value="'.$key['l_user'].'"></div>
		<div class="checkui">日期<input name="l_date" value="'.$key['l_date'].'"></div>
		<div class="checkui">分类名<input name="l_cate" value="'.$key['l_cate'].'"></div>
		<div class="checkui">浏览数<input name="l_view" value="'.$key['l_view'].'"></div>
		<div class="checkui">评论数<input name="l_cmt" value="'.$key['l_cmt'].'"></div>';
	}
	echo $str;
}
?>
...
if(isset($articleinfo)){
	$str = '';
	foreach($articleinfo as $key){
		$str .= '<div class="checkui">用户名<input name="a_user" value="'.$key['a_user'].'"></div>
			<div class="checkui">日期<input name="a_date" value="'.$key['a_date'].'"></div>
			<div class="checkui">分类名<input name="a_cate" value="'.$key['a_cate'].'"></div>
			<div class="checkui">浏览数<input name="a_view" value="'.$key['a_view'].'"></div>
			<div class="checkui">评论数<input name="a_cmt" value="'.$key['a_cmt'].'"></div>';
	}
	echo $str;
}
?>
...
if(isset($pageinfo)){
	...
	foreach($pageinfo as $key){
		$str .= '<div class="checkui">用户名<input name="p_user" value="'.$key['p_user'].'"></div>
			<div class="checkui">日期<input name="p_date" value="'.$key['p_date'].'"></div>
			<div class="checkui">浏览数<input name="p_view" value="'.$key['p_view'].'"></div>
			<div class="checkui">评论数<input name="p_cmt" value="'.$key['p_cmt'].'"></div>';
	}
	echo $str;
}
?>
...
<script>
    $(function(){
    	var opt=$("#PostFILTERCATEGORY").val();
        if(opt=='0'){
            $(".PostFILTERCATEGORY").parent().show();
            $(".PostFILTERCATEGORY").attr("name","PostFILTERCATEGORY");
        }else{
            $(".PostFILTERCATEGORY").parent().hide();
            $(".PostFILTERCATEGORY").removeAttr("name");
        }
        $("#PostFILTERCATEGORY").change(function(){
        	var opt=$("#PostFILTERCATEGORY").val();
            if(opt=='0'){
                $(".PostFILTERCATEGORY").parent().show();
                $(".PostFILTERCATEGORY").attr("name","PostFILTERCATEGORY");
        ...
        <dd class="half<?php if($zbp->Config('tpure')->PostFILTERCATEGORY != '0'){echo " hide";}?>">
            <label for="PostFILTERCATEGORY">过滤的分类ID</label>
            <input type="text" class="PostFILTERCATEGORY"<?php if($zbp->Config('tpure')->PostFILTERCATEGORY == '0'){echo ' name="PostFILTERCATEGORY"';}?> value="<?php echo $zbp->Config('tpure')->PostFILTERCATEGORY;?>" placeholder="不过滤请留空" class="settext" />
            <i class="help"></i><span class="helpcon">请填写首页需要屏蔽的分类ID，<br>多个分类ID之间用英文逗号分隔，首尾不加逗号，<br>不限个数及顺序，留空提交则不过滤。<br></span>
        </dd>
		<dd>
			<label for="PostSHARE">文章分享按钮</label>
			<textarea name="PostSHARE" id="PostSHARE" cols="30" rows="5" class="setinput"><?php echo $zbp->Config('tpure')->PostSHARE;?></textarea>
			<i class="help"></i><span class="helpcon">请填写文章页第三方分享代码，留空则不显示。</span>
		</dd>
		<?php if($zbp->Config('tpure')->PostAJAXON == 1){ ?>
		<dd>
			...
			<i class="help"></i><span class="helpcon">需启用AJAX分页项，设置自动加载N页后手动下一页加载。</span>
		</dd>
		<?php } ?>
		<dd class="half">
			<label>AJAX加载</label>
			<input type="text" id="PostAJAXON" name="PostAJAXON" class="checkbox" value="<?php echo $zbp->Config('tpure')->PostAJAXON;?>" />
			<i class="help"></i><span class="helpcon">“ON”为滚动列表底部自动加载下一页；<br>“OFF”为分页手动进入下一页。</span>
		</dd>
		<dd class="half">
			<label>查看全文按钮</label>
		...
		<dd class="half">
			...
			<i class="help"></i><span class="helpcon">“ON”为个性化时间[格式如：5分钟前]；<br>“OFF”为传统时间[格式如：1970-01-01]；</span>
		</dd>
		<dd class="half">
			<label>返回顶部开关</label>
...
<dl>
	...
	<dd>
		...
		<i class="help"></i><span class="helpcon">“ON”为启用首页/分类/标签/文章自定义SEO信息；<br>“OFF”为关闭自定义SEO信息。</span>
	</dd>
	<?php
	...
	?>
	<input type="hidden" name="SEOTITLE" value="<?php echo $zbp->Config('tpure')->SEOTITLE;?>" />
	<input type="hidden" name="SEOKEYWORDS" value="<?php echo $zbp->Config('tpure')->SEOKEYWORDS;?>" />
	<input type="hidden" name="SEODESCRIPTION" value="<?php echo $zbp->Config('tpure')->SEODESCRIPTION;?>" />
	<?php
	}
	?>
	<dd class="setok"><input type="submit" value="保存设置" class="setbtn" /><img id="statloading" src="style/images/loading.gif" /></dd>
...
<dl>
	...
	<?php }else{ ?>
	<input type="hidden" name="PostCOLOR" value="0188fb" />
	<input type="hidden" name="PostBGCOLOR" value="f6f8f9" />
	<input type="hidden" name="PostSIDELAYOUT" value="r" />
	<input type="hidden" name="PostCUSTOMCSS" value="" />
	<?php } ?>
	<dd class="setok"><input type="submit" value="保存设置" class="setbtn" /><img id="statloading" src="style/images/loading.gif" /></dd>
</dl>
</form>
...
<div class="tfooter">
	<ul>
		...
		<li><a href="http://www.toyean.com/post/tpure.html" target="_blank">帮助说明</a></li>
		<li><a href="http://wpa.qq.com/msgrd?v=3&uin=1151503999&site=qq&menu=yes" target="_blank">技术支持</a></li>
		<li><a href="https://jq.qq.com/?_wv=1027&k=44zyTKi" target="_blank">主题交流群</a></li>
	</ul>
	<p>Copyright &copy; 2010-<script>document.write(new Date().getFullYear());</script> <a href="http://www.toyean.com/" target="_blank">TOYEAN.COM</a> all rights reserved.</p>
</div>

<script type="text/javascript">
	ActiveTopMenu("topmenu_tpure");
</script> 
