...
Main.RightMenu = (function(){
	...
	var selectDesktopSelector = ".menudesktop";	

	var _init = function(){
		$(Main.Config.ToolBar).contextmenu(function(e){return false;});//屏蔽工具栏右键
	...
	var _bindBody = function(){
		$.contextMenu({
			...
			items: {
				"refrech":{name:"刷新(E)",icon:"refresh",accesskey: "e"},
				"sortBy": {
					name: "排序方式(O)",
					accesskey: "o",
					icon:"sort",
				...
				"full":{name:"全屏/退出全屏(F)",icon:"fullscreen",accesskey: "f"},
				"setting_wall":{name:"更改壁纸(W)",icon:"picture",accesskey: "w"},
				"setting":{name:"系统设置(S)",icon:"cogs",accesskey: "s"}
			}
		});
	};
	...
	var _bindDesktop= function(){
		$.contextMenu({
			zIndex:9999,
			selector: selectDesktopSelector, 
			callback: function(key, options) {_menuDesktop(key);},
			items: {
				"open":{name:"打开(O)",icon:"open",accesskey: "o"},
				"open_new":{name:"新窗口打开(N)",icon:"open",accesskey: "o"},
				"info":{name:"属性(R)",icon:"info",accesskey: "r"}
			}
		});
	}
	...
	var _menuDesktop = function(action) {//多选，右键操作
		switch(action){
			case 'open':Main.PathOpen.open();break;
			case 'open_new':Main.PathOpen.open_new();break;			
			case 'info':Main.PathOperate.pathInfo();break;
			default:break;
		}
	};
