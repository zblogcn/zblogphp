Main.FileSelect = (function(){
	...
	var _bindSelectEvent = function(){
		...
		// .unbind('click').live('click',function(e){
		// 	Main.Global.isDragSelect = false;
		// 	stopPP(e);return false;
		// })
		...
		var __dragSelectInit = function(e) {
			...
			if ($(e.target).hasClass("ico")==false){// 编辑状态不可选
				...
				//$selectDiv = $('<div id="selContainer"></div>').appendTo(Main.Config.FileBoxSelector);
				startX = e.pageX - Main.Global.frameLeftWidth;
				startY = e.pageY + $(Main.Config.BodyContent).scrollTop() - offset_top;
				isSelect = true;
			}
		};		
		//框选，鼠标移动中
		var __dragSelecting= function(e) {
			...
			var _w = $selectDiv.width();
			var _h = $selectDiv.height() + Math.abs(scroll_cute);
	...
	var _getPosition = function(pose){
		...
		var __list_position = function(){
			if (Main.Global.fileListSelectNum == 1) {//只有一个为选中状态
				...
				switch(pose){
					case "up":
					case "left":position = ((firstNumber <=0)? firstNumber:firstNumber - 1);
						break;
					case "down":
					case "right":
		...
		var __icon_position = function(){
			...
			if (Main.Global.fileListSelectNum == 1) {//只有一个为选中状态
				...
				switch(pose){
					case "up":
						position = ((firstNumber <=rowNum)? firstNumber:firstNumber-rowNum);
						break;
					case "left":position = ((firstNumber <=0)? firstNumber:firstNumber - 1);
						break;
					case "down":
