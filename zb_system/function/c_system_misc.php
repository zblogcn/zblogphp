...
function misc_statistic()
{
    ...
    if (false == $zbp->option['ZC_LARGE_DATA']) {
        ...
        $all_articles = GetValueInArrayByCurrent($zbp->db->sql->get()->select($GLOBALS['table']['Post'])->count(['*' => 'num'])->where(['=', 'log_Type', '0'])->query, 'num');
        $all_pages = GetValueInArrayByCurrent($zbp->db->sql->get()->select($GLOBALS['table']['Post'])->count(['*' => 'num'])->where(['=', 'log_Type', '1'])->query, 'num');
        $all_members = GetValueInArrayByCurrent($zbp->db->sql->get()->select($GLOBALS['table']['Member'])->count(['*' => 'num'])->query, 'num');
    ...
    if (true == $zbp->option['ZC_LARGE_DATA'] || true == $zbp->option['ZC_VIEWNUMS_TURNOFF']) {
        $all_views = 0;
    } else {
        $all_views = GetValueInArrayByCurrent($zbp->db->sql->get()->select($GLOBALS['table']['Post'])->sum(['log_ViewNums' => 'num'])->query, 'num');
    }
    //一直统计
    $all_categories = GetValueInArrayByCurrent($zbp->db->sql->get()->select($GLOBALS['table']['Category'])->count(['*' => 'num'])->query, 'num');
    $all_tags = GetValueInArrayByCurrent($zbp->db->sql->get()->select($GLOBALS['table']['Tag'])->count(['*' => 'num'])->query, 'num');
    ...
    try {
        if ($zbp->build_system_module) {
            $zbp->AddBuildModule('statistics', [$all_articles, $all_pages, $all_categories, $all_tags, $all_views, $all_comments]);
...
function misc_showtags()
{
    ...
    $array = $zbp->GetTagList(null, ['=', 'tag_Type', $type], ['tag_Count' => 'DESC', 'tag_ID' => 'ASC'], [100], null);
    if (count($array) > 0) {
        $t = [];
...
function misc_php_zbp_info()
{
    ...
    foreach ($GLOBALS as $n => $v) {
        if (false === strpos($n, 'Filter_Plugin_')) {
            if ('integer' == gettype($v) || 'double' == gettype($v) || 'string' == gettype($v) || 'boolean' == gettype($v)) {
