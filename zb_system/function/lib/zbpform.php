...
zbpform::zbcheckbox('aaaa',array('选项1', '选项2'),1);
...
class zbpform {
	...
	public static function checkbox($name, $array, $ischecked = 0) {
		...
		$s .= "<input type=\"checkbox\" name=\"$name\" id=\"$name\" class=\"$name\" value=\"1\"$checked />$checkedtext \r\n";
		echo $s;
	}

	public static function zbcheckbox($name, $array, $ischecked = 0) {
		$s = '';
		$checked = $ischecked ? ' checked="checked"' : '';
		$checkedtext = $array[$ischecked];
		$s .= "<input name=\"$name\" id=\"$name\" class=\"$name checkbox\" style=\"display:none;\" type=\"text\" value=\"1\"$checked >$checkedtext \r\n";
		echo $s;
	}
	...
	public static function textarea($name, $value, $width = 250,  $height = 150) {
